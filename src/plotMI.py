...
def plotMI():
    ...
    p = args.p*J #pseudocount mass added to k-mers
    ...
    res = [pool.apply_async(getP_j,args=(seqs,j,I,J,args.k,p)) for j in range(0,J-args.k+1)]
    ...
    for m in range(0,J-2*args.k+1):
        M += 1
        for n in range(m+args.k,J-args.k+1): res.append(pool.apply_async(getMI_mn,args=(seqs,m,n,I,J,P,args.k,p)))
    ...
    print(MI)
    print(ma.masked_array(MI,mask=mask))
    sns_plot = sns.heatmap(MI,cmap='Blues',cbar=True,cbar_kws={'label': 'MI'},xticklabels=xticks,yticklabels=yticks)
